...
class TTSWrapper:
    ...
    def _save_wav(self, output_file, wav, sample_rate: int = 24000):
        """Save mono PCM16 WAV without torchaudio/torchcodec dependencies.
        Accepts torch.Tensor or numpy array in range [-1, 1].
        """
        try:
            ...
            logger.error(f"Failed to save wav to {output_file}: {e}")
            raise
    ...
    async def stream_generation(self,text,speaker_name,speaker_wav,language,accent,output_file):
        ...
        if len(file_chunks) > 0:
            ...
            # Save using wave module to avoid torchcodec/ffmpeg
            self._save_wav(output_file, wav.cpu().squeeze().numpy(), 24000)
        else:
    ...
    def local_generation(self,text,speaker_name,speaker_wav,language,accent,output_file):
        ...
        # Save using wave module to avoid torchcodec/ffmpeg
        self._save_wav(output_file, out["wav"], 24000)
