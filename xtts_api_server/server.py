...
if STREAM_MODE or STREAM_MODE_IMPROVE:
  ...
  XTTS.load_model()
  try:
      logger.info(f"CUDA available: {torch.cuda.is_available()} | device set: {XTTS.device}")
      if torch.cuda.is_available() and str(XTTS.device).lower() != 'cpu':
          # 做一次简单的张量分配验证
          _t = torch.zeros((1,), device='cuda')
          logger.info(f"CUDA warmup tensor device: {_t.device}")
  except Exception as _e:
      logger.warning(f"CUDA self-check failed: {_e}")
...
def get_speakers_list():
    """Mantella 期望的返回结构：{"lang": {"speakers": [speaker, ...]}}
    统一输出嵌套结构，确保客户端使用 .get('speakers') 不会抛错。
    """
    raw = XTTS.get_speakers()
    result = {}
    try:
        if isinstance(raw, list):
            # 老实现：扁平列表 → 包装成 zh-cn.speakers
            names = [n for n in raw if isinstance(n, str)]
            cleaned = sorted({n.strip() for n in names if n.strip() and not n.strip().startswith('.')})
            result["zh-cn"] = {"speakers": cleaned}
        elif isinstance(raw, dict):
            for lang, val in raw.items():
                if isinstance(val, dict) and isinstance(val.get("speakers"), list):
                    names = [n for n in val.get("speakers", []) if isinstance(n, str)]
                elif isinstance(val, list):
                    names = [n for n in val if isinstance(n, str)]
                else:
                    names = []
                cleaned = sorted({n.strip() for n in names if n.strip() and not n.strip().startswith('.')})
                result[lang] = {"speakers": cleaned}
        else:
            result["zh-cn"] = {"speakers": [str(raw)]}
    except Exception as e:
        logger.warning(f"/speakers_list 格式兼容处理异常: {e}")
        result = {"zh-cn": {"speakers": (raw if isinstance(raw, list) else [])}}
    return result
...
def health():
    try:
        return {"status": "ok", "device": XTTS.device, "cuda": bool(torch.cuda.is_available()), "model_version": MODEL_VERSION}
    except Exception as e:
